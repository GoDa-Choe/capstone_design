...
for epoch in range(NUM_EPOCH):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        # print("target", target)
        # correct += (predictions == target).sum().item()  # accumulate correct output per mini-batch
        # count += target.size(0)
        #
    ...
    torch.save(classifier.state_dict(), '%s/cls_model_%d.pth' % (OUTPUTFILE_DIRECTORY, epoch))
    scheduler.step()
