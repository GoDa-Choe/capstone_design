...
if __name__ == "__main__":
    ...
    generator = PCN(emb_dims=1024, input_shape='bnc', num_coarse=1024, grid_size=4, detailed_output=False)
    generator.to(device=DEVICE)

    optimizer = optim.Adam(generator.parameters(), lr=LEARNING_RATE, betas=BETAS)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=STEP_SIZE, gamma=GAMMA)

    log_file = get_log_for_auto_encoder(dataset_type="mvp", loss_type="cd_pcn")
    weights_directory = get_trained_model_directory_for_auto_encoder(dataset_type="mvp", loss_type="cd_pcn")
