...
class Partition:
    ...
    def eight_axis_partition(self):
        ...
        time.sleep(0.1)
        print(f"{self.raw_dataset.incomplete_pcds.shape} -> ({len(partitioned_pcds)}, 2048, 3)\n")
...
class Partition_for_Test:
    ...
    def select_pcds_sample_points(self, partitioned_pcds):
        ...
        print(f"Partitioned_pcds is being selected and sampled...")
        occluded_pcds = []
        for i in tqdm(range(0, len(partitioned_pcds), self.num_partition)):
            candidate_pcds = [partitioned_pcd for partitioned_pcd
                              in self.partitioned_pcds[i:i + self.num_partition]
                              if len(partitioned_pcd) >= self.num_points]

            random.shuffle(candidate_pcds)
            selected_pcds = candidate_pcds[:self.num_select]
            self.sample_points(selected_pcds)
...
if __name__ == "__main__":
    ...
    # raw = RawDatasetLoader(file_name="MVP_Validation.h5", directory="data/mvp/", for_test=False)
    raw = RawDatasetLoader(file_name="MVP_Train.h5", directory="data/mvp/", for_test=False)

    # partition = Partition_for_Test(raw, partition_type="8-axis", num_select=1, num_points=256)
    partition = Partition(raw, partition_type="8-axis", num_select=1, num_points=256)
    partition.save(directory='temp', file_name="Partitioned_MVP_Train.h5")
